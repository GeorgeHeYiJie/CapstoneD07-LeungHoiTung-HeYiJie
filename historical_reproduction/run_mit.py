"""One isolated archival MIT run with the original label-only physics-loss defect."""
import csv
import hashlib
import io
import json
...
SOURCE = ROOT / 'historical_reproduction/source'
...
OUT = ROOT / ('results/smoke_tests/historical_mit_01' if SMOKE else 'results/reproduction/MIT/historical_run_01')
...
def main():
    ...
    args.early_stop = 20  # Archived logs, overriding released script's default 10.
    if SMOKE:
        args.epochs = 3
    import main_MIT, Model.Model, dataloader.dataloader, utils.util
    modules = [main_MIT, Model.Model, dataloader.dataloader, utils.util]
    assert all(Path(m.__file__).resolve().is_relative_to(SOURCE.resolve()) for m in modules)
    manifest = json.loads((ROOT/'historical_reproduction/source_manifest.json').read_text())
    for name, digest in manifest['sha256'].items():
        assert hashlib.sha256((SOURCE/name).read_bytes()).hexdigest() == digest
    protected = json.loads((ROOT/'historical_reproduction/protected_before.json').read_text())
    assert all(hashlib.sha256((ROOT/name).read_bytes()).hexdigest() == digest for name,digest in protected.items())
    assert (args.epochs, args.batch_size, args.alpha, args.beta) == (3 if SMOKE else 200, 512, 1, 50)
    source_paths = ['historical_reproduction/source/' + name for name in manifest['sha256']] + ['historical_reproduction/run_mit.py']
    ...
    config = dict(arguments=vars(args), experiment_count=1, started_utc=started,
        ...
        historical_commit=manifest['commit'], historical_loss='sum ReLU(y2-y1); label-only, zero parameter gradient',
        run_kind='three-epoch smoke test' if SMOKE else 'one full historical experiment',
        loaded_modules={m.__name__:m.__file__ for m in modules},
    ...
    label_penalty = torch.relu(loaders['train'].dataset.tensors[3] - loaders['train'].dataset.tensors[2])
    assert not label_penalty.requires_grad
    config['expected_physics_loss_epoch_average'] = float(label_penalty.sum()) / len(loaders['train'])
    dataset_files = config['split']['training_and_validation_battery_files'] + config['split']['test_battery_files']
    write_json('dataset_sha256.json', {name:hashlib.sha256((ROOT/name).read_bytes()).hexdigest() for name in dataset_files})
    config['tensor_sha256'] = {k: [hashlib.sha256(t.numpy().tobytes()).hexdigest()
                                      for t in v.dataset.tensors] for k,v in loaders.items()}
    current_config = json.loads((ROOT/'results/reproduction/MIT/run_01/run_configuration.json').read_text())
    assert config['tensor_sha256'] == current_config['tensor_sha256']
    config['all_dataset_tensors_identical_to_current_run_01'] = True
    model = RecordedPINN(args)
    initial_solution = {k:v.detach().clone() for k,v in model.solution_u.state_dict().items()}
    ...
    assert any(not torch.equal(v, model.solution_u.state_dict()[k]) for k,v in initial_solution.items())
    assert all(abs(r['physics_loss']-config['expected_physics_loss_epoch_average']) < 1e-7 for r in model.history)
    assert all(hashlib.sha256((ROOT/name).read_bytes()).hexdigest() == digest for name,digest in protected.items())
    write_json('preservation_check.json', dict(all_26_protected_files_unchanged=True, sha256=protected))
    ...
    ax.set(xlabel='True SOH (%)', ylabel='Predicted SOH (%)',
           title=f'Historical MIT run 01: best validation epoch {result["best_epoch"]}')
    ...
    result['label_only_physics_verified'] = True
    result['optimizer_update_verified'] = True
